export const branch = {
  title: "Branches",
  subtitle: "List from API:",
  listTitle: "Branches",
  listDesc:
    "Season and headcount use the server date (today). «Not started» means assigned staff with a future hire date.",
  listSearchPlaceholder: "Search by name, id, address, managers…",
  listSearchNoResults: "No branches match your search.",
  openedFromReportBanner:
    "You opened this branch from a report; the detail sheet below shows that branch and its daily movements.",
  add: "Add branch",
  addTitle: "New branch",
  addHint:
    "Name is required (max 100 characters). POS beneficiary is required; for franchise, joint venture, or other, the note must state where takings go.",
  addPosSettlementLegend: "POS / card takings — who is the beneficiary?",
  addPosSettlementLead:
    "Required. Pick who card/POS takings accrue to in reporting. For franchise, joint venture, or other, you must spell out the economic destination in the note.",
  posSettlementPatron: "Patron / company",
  posSettlementPatronHint: "Card/POS takings are labeled as accruing to the owner/company.",
  posSettlementFranchise: "Franchise",
  posSettlementFranchiseHint:
    "Takings are attributed to the franchise operator side. Royalties, settlement splits, or bank routing are not derived from this choice alone — add contract or account detail in notes.",
  posSettlementJoint: "Joint venture / partnership",
  posSettlementJointHint:
    "Takings are attributed to the joint-venture model. Partner share or payout account is not configured here — spell out the arrangement in notes.",
  posSettlementBranchPersonnel: "Branch operator (named staff)",
  posSettlementBranchPersonnelHint:
    "A named staff member appears as payee in reports (pick them in Branches → POS settlement profile after assigning staff to this branch).",
  posSettlementOther: "Other",
  posSettlementOtherHint:
    "Use for mixed or bespoke structures; explain who or which entity is the payee in notes.",
  posBranchPersonnelFollowUp:
    "After you assign staff to this branch, open Branches → quick actions → POS settlement profile and pick the payee person if needed.",
  posSettlementNotesLabel: "Notes (optional)",
  posSettlementNotesPlaceholder: "e.g. contract reference, operating model detail…",
  posSettlementNotesPlaceholderRequired:
    "Required: who or which entity receives POS takings? (e.g. franchise legal name, partner share…)",
  posSettlementNotesRequired:
    "For franchise, joint venture, or other, enter a note that states where takings economically go.",
  posSettlementNotesRequiredHint:
    "The app does not infer a single payee for these three types — document the destination here.",
  posSettlementNotesMaxLength: "At most 500 characters.",
  edit: "Edit branch",
  editTitle: "Edit branch",
  editHint: "PUT /api/branches/{id} — name, address, and branch managers (assigned staff only).",
  fieldName: "Branch name",
  fieldAddress: "Address",
  fieldResponsibles: "Managers",
  responsiblesHint:
    "Only staff assigned to this branch can be selected. They become branch managers; if their login is linked to that personnel record, Manage user scopes will imply personnel data access for everyone in this branch (and warehouse shipment rules use the same link).",
  responsiblesEmptyStaff: "No assigned staff yet — assign personnel to this branch first.",
  responsiblesScopePreviewTitle: "Personnel data preview",
  responsiblesScopePreviewBody:
    "Each selected manager, when their login is linked to that personnel record, can see all staff listed below in personnel data scope.",
  responsiblesScopePreviewSelectHint: "Select at least one manager to preview the covered staff list.",
  responsiblesScopePreviewCount: "{count} people",
  detailResponsibles: "Managers",
  tableId: "Id",
  tableName: "Name",
  tableSeason: "Tourism season",
  tableSeasonHint:
    "From tourism season periods (server today). Open: opened on or before today and not closed or closed on/after today.",
  tableStaff: "Staff",
  tableStaffHint:
    "Assigned headcount. «Not started» means hire date is after today (server date).",
  seasonOpen: "Open",
  seasonPlanned: "Opening pending",
  seasonClosed: "Closed",
  seasonNone: "No record",
  staffLineNone: "—",
  staffLineAllActive: "{total} assigned · {n} active",
  staffLineMixed: "{total} assigned · {started} active · {pending} not started yet",
  staffLinePendingOnly: "{total} assigned · not started yet ({pending})",
  tableActions: "Actions",
  listSortLabel: "Sort by",
  listSortNameAsc: "Name (A-Z)",
  listSortNameDesc: "Name (Z-A)",
  listSortIdAsc: "Id (ascending)",
  listSortIdDesc: "Id (descending)",
  listSortStaffDesc: "Staff count (high to low)",
  listMetricsToggle: "Income and scoped stock inbound",
  /** Short heading on mobile branch cards; full phrase stays on the second line. */
  listMetricsMobileTitle: "Income & stock summary",
  /** Shown above money rows in the expandable branch list summary. */
  listMetricsCurrencyCaption: "Amounts are in TRY.",
  listMetricsLoading: "Loading summary…",
  listTodayIncome: "Today’s income (UTC)",
  listTotalIncome: "All-time income",
  listAllTimeProfit: "Profit to date (income − expense)",
  listStockInboundScopeTitle: "Stock inbound total (pick scope)",
  listStockInboundScopeExplainer:
    "Same rules as warehouse/stock reports: subcategories first; parent category includes its subcategories.",
  listStockInboundScopeTotal: "Inbound qty (selected scope)",
  listStockInboundScopeHint: "Sum of positive branch stock receipts for products in the scope above (all time).",
  listStockInboundScopePickHint: "Choose a category, product group, or SKU to load this total.",
  listMetricsPersonnelNote: "Financial summary hidden; use the scope above for inbound qty.",
  quickAddIncome: "Add income (register)",
  quickAddExpense: "Add expense (register)",
  quickActions: "Quick register actions",
  quickMenuStoryBranch: "Branch",
  quickCurrentAccount: "Current account / receipt",
  quickMenuStoryRegister: "Branch register",
  quickMenuStoryStock: "Stock",
  quickMenuStoryPersonnelReports: "Staff & reports",
  quickMenuStoryDangerZone: "Danger zone",
  deleteBranch: "Delete branch",
  deleteBranchConfirmTitle: "Delete this branch?",
  deleteBranchConfirmBody:
    "“{name}” will be soft-deleted (hidden from lists; support can restore if needed). This is only allowed when the branch has no assigned active personnel.",
  deleteBranchConfirm: "Yes, delete",
  quickAddDayClose: "Add day close",
  listRowPdfSettlement:
    "PDF report (advances, expenses, notes — by branch)",
  listRowPosProfile: "POS settlement profile (beneficiary)",
  posProfileUnsetHint:
    "No profile saved for this branch yet — pick a beneficiary and save.",
  posProfileReportsHint:
    "Used on Patron flow and related views for the POS / card takings label.",
  branchPdfOptionsTitle: "Branch PDF — what to include?",
  branchPdfOptionsIntro:
    "Choose which blocks to load and how stock is shown. Checked sections appear in the report and drive the summary lines.",
  branchPdfOptStockInbound: "Stock received at the branch (warehouse → branch)",
  branchPdfOptStockPricing:
    "Show estimated cost columns (from latest warehouse IN invoice line for that product/warehouse; empty if none)",
  branchPdfOptStockGroupByParent: "Group stock inbound by main product",
  branchPdfOptStockGroupByParentHint:
    "Instead of line-by-line, show total quantity, variant count and (if enabled) estimated value under each main product.",
  branchPdfStockColParentProduct: "Main product",
  branchPdfStockColVariety: "Variants",
  branchPdfStockColGroupValueEst: "Estimated value (total)",
  branchPdfOptAdvances: "Advances tied to this branch",
  branchPdfOptPersonnelExpenses:
    "Personnel-tagged expense lines (non-advance, on the branch register)",
  branchPdfOptPersonnelSalary: "Salary / social-cost estimate table (branch personnel)",
  branchPdfOptRegister: "Branch register — all income and expense lines",
  branchPdfOptNotes: "Branch notes",
  branchPdfOverlapHint:
    "Note: with both «Personnel-tagged expenses» and «Full register» selected, the same lines may appear in both lists; do not double-count totals.",
  branchPdfConfirm: "Generate PDF",
  branchPdfSectionStock: "Stock inbound to branch",
  branchPdfColWarehouse: "Warehouse",
  branchPdfColQty: "Quantity",
  branchPdfColUnit: "Unit",
  branchPdfColUnitPrice: "Unit price (estimate)",
  branchPdfColLineValueEst: "Line value (estimate)",
  branchPdfStockValuationFootnote:
    "Unit price and line value use the latest supplier invoice linked to a warehouse IN for the same product and warehouse; actual shipment cost may differ.",
  branchPdfSectionRegister: "Branch register",
  branchPdfRegisterColType: "Flow",
  branchPdfRegisterTypeIn: "Income",
  branchPdfRegisterTypeOut: "Expenses / outflows",
  branchPdfRegisterDayCloseBadge: "Day-close",
  branchPdfTransferBadge: "Transfer",
  branchPdfTransferExcludedNote:
    "Rows marked “Transfer” (owner hand-over, pocket/held settlement, non-P&L) are cash movements within the register; they are excluded from the expense/profit totals in the summary above.",
  branchPdfDistOther: "Other",
  branchPdfDistCapPayType: "Payment type",
  branchPdfDistCapIncomeType: "Collected as",
  branchPdfDistCapCashHolder: "Cash held by",
  branchPdfDistCapPaySource: "Payment source",
  branchPdfDistCapAdvanceSource: "Advance source",
  branchPdfCashHolderPatron: "Owner",
  branchPdfCashHolderBranch: "Branch register",
  branchPdfCashHolderPersonnel: "Personnel-held",
  branchPdfCashHolderUnassigned: "Unassigned / other",
  branchPdfSummaryTitle: "Branch summary — net",
  branchPdfSourceBreakdownTitle: "Expense totals by payment source and category",
  branchPdfSourceBreakdownNote: "(branch + personnel expense + advances)",
  branchPdfSourceBranchExpense: "Branch expense",
  branchPdfSourcePersonnelExpense: "Personnel expense + advances",
  branchPdfOutflowTitle: "Personnel costs — advances + expenses",
  branchPdfOutflowPerPersonAdvance: "Advance total per person",
  branchPdfOutflowColType: "Type",
  branchPdfOutflowColDetail: "Detail",
  branchPdfOutflowColSource: "Source",
  branchPdfOutflowBadgeAdvance: "Advance",
  branchPdfOutflowBadgeExpense: "Expense",
  branchPdfOutflowSubtotalAdvance: "Advances",
  branchPdfOutflowSubtotalExpense: "Expenses",
  branchPdfOutflowSubtotalTotal: "Total",
  branchPdfSourceGrandTotal: "Grand total (all sources)",
  branchPdfDayCloseCashTitle: "Day-close cash — held by",
  branchPdfBranchCashWhereTitle: "Where the branch cash is (day-close)",
  branchPdfBranchCashTotal: "Total",
  branchPdfCashWhereWho: "Who / where",
  branchPdfCashReceived: "Received",
  branchPdfCashRemaining: "Remaining",
  branchPdfAdvancesWhoFromTitle: "Staff advances — who / from where",
  branchPdfNoRows: "No records",
  branchPdfTotalStockValuationEst: "Inbound stock — estimated cost (total)",
  branchPdfStockQtySummary: "Stock (quantity)",
  branchPdfTotalInboundQty: "Total inbound quantity",
  branchPdfTotalRegisterIn: "Register income (total)",
  branchPdfTotalRegisterOut: "Register expense (total)",
  branchPdfTotalRegisterNet: "Register net (in − out)",
  branchPdfTotalRegisterSpent: "Spent from register",
  branchPdfTotalRegisterSpentShort: "From register",
  branchPdfTotalRegisterNetCash: "Register net (in − from register)",
  branchPdfTotalPatronSpent: "From owner's pocket",
  branchPdfTotalPersonnelHeldSpent: "From personnel-held cash",
  branchPdfTotalPersonnelPocketSpent: "From personnel pocket",
  branchPdfPnlRevenue: "Revenue",
  branchPdfPnlGrossRevenue: "Gross revenue",
  branchPdfPnlCashEarned: "Earned cash",
  branchPdfPnlCashInHand: "Cash in hand",
  branchPdfPnlNetRevenue: "Net revenue (register)",
  branchPdfPnlExpenses: "Branch expense (by source)",
  branchPdfPnlTotalExpense: "Total branch expense",
  branchPdfPnlProfit: "Profit (revenue − branch expense)",
  branchPdfPnlScopeNote:
    "The summary above shows only the branch's expense and profit. Personnel expense and advances are below; remaining cash is in the “Where the branch cash is” card.",
  branchPdfPnlBranchSpend: "Spent for this branch",
  branchPdfPnlAdvancesToStaff: "Advances to staff",
  branchPdfModeDetail: "Detail (line by line)",
  branchPdfModeSummary: "Summary (totals only)",
  branchPdfOptCurrentAccount: "Branch current account (shipments / invoices)",
  branchPdfOptCurrentAccountHint:
    "Invoice totals billed to this branch, collections and open balance. No line detail.",
  branchPdfCariTitle: "Branch current account — shipments / invoices",
  branchPdfCariColShipment: "Document / shipment",
  branchPdfCariColTotal: "Amount",
  branchPdfCariColCollected: "Collected",
  branchPdfCariColOpen: "Open",
  branchPdfCariColStatus: "Status",
  branchPdfCariStatusPaid: "Collected",
  branchPdfCariStatusPartial: "Partial",
  branchPdfCariStatusOpen: "Open",
  branchPdfCariGrandTotal: "Grand total",
  branchPdfSectionGroupBranch: "Branch expense",
  branchPdfSectionGroupPersonnel: "Personnel expense",
  branchPdfSectionGroupStock: "Warehouse info",
  branchPdfSectionGroupOther: "Other",
  branchPdfOptPersonnelCombined: "Personnel advances + expenses",
  branchPdfPersonnelCombinedListMode: "Advances + expenses: list format",
  branchPdfAdvancesListMode: "Advances: list format",
  branchPdfPersonnelExpensesListMode: "Personnel-tagged expenses: list format",
  branchPdfRegisterListMode: "Branch register: list format",
  branchPdfSummaryColCurrency: "Currency",
  branchPdfSummaryColTotal: "Total",
  branchPdfSummaryColCount: "Line count",
  branchPdfSummarySublineAdvances:
    "{n} advance record(s) — detailed rows not printed.",
  branchPdfSummarySublineExpenses:
    "{n} expense line(s) — detailed rows not printed.",
  branchPdfRegisterSummaryCounts: "{in} income line(s), {out} expense line(s).",
  listRowAddPersonnel: "Add personnel to branch",
  noData: "No branches yet.",
  selectHint:
    "Tap the card or the Details button — the branch summary opens full screen.",
  openDetail: "Open",
  closeDetail: "Close",
  detailDesc:
    "Overview, personnel, register income/expense lines, and warehouse-to-branch stock — use tabs.",
  detailTabsAria: "Branch detail tabs",
  dashSubTabsAria: "Summary sub-tabs",
  dashSubTabDay: "Day",
  dashSubTabCash: "Cash position",
  dashSubTabOverview: "Overview",
  dashSubTabMonth: "Month",
  dashSubLeadDay: "The selected day's register activity and daily profit/expense breakdown.",
  dashSubLeadCash: "As of today: cash in the drawer and who owes whom (patron / personnel pocket).",
  dashSubLeadOverview: "Branch health at a glance: all-time and today's summary figures.",
  dashSubLeadMonth: "Income/expense trend for the selected report month.",
  dashSectionDay: "Selected day",
  registerSummaryBlurb:
    "The date above updates both the summary cards and the daily list below.",
  registerSummaryStoryTitle: "How should I read this?",
  registerSummaryStory1:
    "Start with Totals: Cash in drawer is physical cash; the two cards next to it are cumulative net amounts owed to staff and to you — the big picture is here.",
  registerSummaryStory2:
    "Then Day summary: Operating net is that day’s accounting result (income − accounting expense). The two net debt cards are only that day’s pocket and owner-related movements.",
  registerSummaryStory3:
    "Income and Expense (accounting) are building blocks of operating net; if something looks off, verify against the line list below.",
  registerSummaryBadgePriority: "Focus",
  registerSummaryBadgeDayDebt: "Day — net debt",
  registerTotalsSectionLead: "Big picture — look at these three first.",
  registerTodaySectionLead: "Selected day — review after totals.",
  registerTodaySection: "Day summary",
  registerTodayIncome: "Income",
  registerTodayExpenseAccounting: "Expense (accounting)",
  registerTodayExpenseAccountingHint: "Cash out from register + off-register advances booked that day.",
  registerTodayNet: "Operating net (day)",
  registerTodayNetHint: "Income − accounting expense. Patron/personnel-pocket lines are not in this card.",
  registerTodayNetPersonnelPocket: "Net owed to staff (day)",
  registerTodayNetPersonnelPocketHint: "Paid from pocket − pocket repayments the same day.",
  registerTodayNetPatron: "Net owed to owner (day)",
  registerTodayNetPatronHint: "Owner-paid expenses − register repayments to owner that day.",
  dashDailySnapshotSection: "Daily snapshot",
  dashDailyProfitTitle: "Daily profit (accounting)",
  dashDailyProfitHint:
    "Income minus accounting expense (cash out from register plus off-register advances booked that day). Same figure as “Operating net (day)” below.",
  dashTopExpenseTypeTitle: "Largest expense type today",
  dashTopExpenseTypeHint: "By main category on economic expense lines; pocket/owner debt repayments excluded.",
  dashTopExpenseNone: "No expenses in this scope today.",
  registerTotalsSection: "Totals (through selected day)",
  registerTotalCashInDrawer: "Cash in drawer",
  registerTotalCashInDrawerHint: "Through selected date: cash in − out from register.",
  registerTotalPersonnelPocketNet: "Net owed to staff",
  registerTotalPersonnelPocketNetHint:
    "Cumulative pocket-related movements from the start through the selected date (net).",
  registerTotalPatronNet: "Net owed to owner",
  registerTotalPatronNetHint:
    "Cumulative owner-paid expenses minus register repayments through the selected date (net).",
  registerPatronNetNegativeMeansBranchReceivable:
    "A negative amount means the branch is owed by the owner.",
  dashSectionLive: "Live snapshot (not tied to the date above)",
  dashSectionLiveHint:
    "Headcount, all-time income/expense, cash accumulated in the register through today, and today’s calendar-day income/expense. The date picker above does not change these.",
  dashStockInboundSection: "Stock inbound by category / product",
  dashStockInboundSectionHint:
    "Totals positive quantities received at this branch for the selected scope (all time). Changing the filters reloads the figure automatically.",
  dashStockInboundTotal: "Inbound quantity (scope)",
  dashStockInboundTotalHint: "Matches warehouse product scope filters (category includes subcategories).",
  dashStockInboundPickScope: "Select at least one filter (category, main product, or SKU) to see the total.",
  dashStockOpenDetailTab: "Line list (Stock tab)",
  registerDayBookIncomeOnlyTitle: "Income lines (day)",
  registerDayBookIncomeOnlyHint:
    "Only register income rows are listed below. Expenses and other movements are on desktop layout or under Expenses.",
  dashMobileNoIncomeForDay: "No income rows for this date.",
  dashSectionMonth: "By report month",
  dashSectionMonthHint: "Income and expense totals for the chosen calendar month only.",
  tabDashboard: "Overview",
  tabPersonnel: "Personnel",
  tabCurrentAccount: "Current account",
  personnelSubTabsAria: "Personnel section tabs",
  personnelSubTabPeople: "Staff list",
  personnelSubTabAdvances: "Advances given",
  personnelSummaryCardAssigned: "Assigned to branch",
  personnelSummaryCardStarted: "Started (hire date)",
  personnelSummaryCardAdvancesLifetime: "Total advances (all-time)",
  personnelSummaryCardAdvancesFiscalYear: "Advances (FY {year})",
  personnelSummaryFiscalAdvancesCountHint: "{n} advance record(s)",
  personnelSummaryCardPocketPaid: "Paid from pocket (total)",
  personnelSummaryCardNetRegisterOwes: "Net owed to staff (pocket)",
  tabIncome: "Income",
  tabExpenses: "Expenses",
  tabStock: "Branch warehouse",
  tabTourismSeason: "Tourism season",
  tabZReportAccounting: "Z report (accounting)",
  tabNotes: "Notes",
  tabDocuments: "Documents",
  currentAccountHint:
    "Track invoices issued to this branch, collected amounts, and remaining open balance here. The PDF button is available only for invoices saved to system documents.",
  uninvoicedShipmentsBadgeTitle: "{{count}} shipment(s) not invoiced",
  uninvoicedShipmentsBannerTitle:
    "{{shipments}} shipment(s) not yet invoiced ({{lines}} product line(s)).",
  uninvoicedShipmentsBannerHint:
    "Shipments sent to this branch have remaining quantity with no invoice. Create invoices to keep the current account accurate.",
  uninvoicedShipmentsShowList: "Show uninvoiced shipments",
  uninvoicedShipmentsHideList: "Hide list",
  uninvoicedShipmentInvoiceCta: "Invoice",
  uninvoicedShipmentInvoiceMissingWarehouse:
    "This shipment's warehouse could not be found; cannot open the invoicing page.",
  currentAccountInvoicedTotal: "Total invoiced",
  currentAccountPaidTotal: "Total collected",
  currentAccountOpenTotal: "Open balance",
  currentAccountEmpty: "No current-account invoices found for this branch.",
  currentAccountColDate: "Date",
  currentAccountColInvoiceNo: "Invoice no",
  currentAccountColInvoiceTotal: "Invoice amount",
  currentAccountColPaid: "Collected",
  currentAccountColPromoDiscount: "Promo/discount",
  currentAccountColPromo: "Promo",
  currentAccountColPromoMoney: "Cash",
  currentAccountColGiftAmount: "Gift",
  currentAccountColAdvance: "Advance",
  currentAccountColOpen: "Open",
  currentAccountColPdf: "PDF",
  currentAccountColPdfStatus: "PDF status",
  currentAccountColReceiptImageStatus: "Receipt image",
  currentAccountColActions: "Actions",
  currentAccountPdfStatusSaved: "PDF ready",
  currentAccountPdfStatusMissing: "No PDF",
  currentAccountPdfMissing: "N/A",
  currentAccountPdfView: "View",
  currentAccountPdfDownload: "Download",
  currentAccountReceiptImageStatusSaved: "Receipt uploaded",
  currentAccountReceiptImageStatusMissing: "No receipt image",
  currentAccountReceiptImageView: "View receipt image",
  currentAccountReceiptImageDownload: "Download receipt image",
  currentAccountAddReceipt: "Add receipt",
  currentAccountReceiptModalTitle: "Invoice receipt",
  currentAccountReceiptDate: "Receipt date",
  currentAccountReceiptAmount: "Receipt amount",
  currentAccountReceiptFillOpenAmount: "Fill full open amount",
  currentAccountReceiptNote: "Note",
  currentAccountReceiptImage: "Bank transfer receipt image",
  currentAccountSaveReceipt: "Save receipt",
  currentAccountInvalidReceiptAmount: "Enter a valid receipt amount.",
  currentAccountReceiptSaved: "Receipt saved.",
  currentAccountCollectedBadge: "Collected",
  currentAccountNoOpenInvoicesForAllocation:
    "No open invoices in this currency to allocate the receipt.",
  currentAccountReceiptDistributedSaved:
    "Receipt distributed across {n} invoice(s), total {amount}.",
  currentAccountReceiptUnappliedRemainder:
    "Open debt total is lower than payment; {amount} could not be applied.",
  currentAccountExportPdf: "Download branch current-account PDF",
  currentAccountPdfDocumentTitle: "Branch current account summary",
  currentAccountPdfGeneratedAt: "Generated",
  currentAccountPdfScope: "Scope",
  currentAccountPdfScopeBranchOnly: "Selected branch only",
  currentAccountPdfTotals: "Totals (invoice/paid/open)",
  currentAccountPdfOptionsTitle: "Branch current-account PDF settings",
  currentAccountPdfOptionsHint:
    "Choose which fields to include in the PDF. You can preview first, then download.",
  currentAccountPdfShowCompanyName: "Show company name",
  currentAccountPdfShowLogo: "Show logo",
  currentAccountPdfShowIban: "Include IBAN / payment details",
  currentAccountPdfRecordsTitle: "Current-account rows to include",
  currentAccountPdfSelectAll: "Select all",
  currentAccountPdfNoSelection: "Select at least one current-account row for the PDF.",
  currentAccountPdfIbanAccountHolder: "Account holder",
  currentAccountPdfIbanBankName: "Bank name",
  currentAccountPdfIbanNote: "Note (optional)",
  currentAccountPdfPreview: "Generate preview",
  currentAccountPdfExport: "Download PDF",
  currentAccountPdfPreviewHint: "Click “Generate preview” to render the PDF preview.",
  currentAccountSubTabsAria: "Current account sub-tabs",
  currentAccountSubTabInvoices: "Invoices",
  currentAccountSubTabReceipts: "Receipts",
  currentAccountSubTabLedger: "Ledger balance",
  ledgerHint:
    "Customer's total charges, payments received and remaining balance — running-ledger model.",
  ledgerTotalCharged: "Total charged (invoices)",
  ledgerTotalPaid: "Total paid",
  ledgerOpenBalance: "Open balance",
  ledgerAddGeneralReceipt: "Add receipt",
  ledgerModalApplyAll: "All",
  ledgerModalApplyOpenBalance: "Apply open balance to amount",
  currentAccountReceiptImageBranchOnly: "Receipt image can only be uploaded for branch counterparties.",
  currentAccountReceiptImageUploadCta: "Choose a file or capture with camera",
  currentAccountReceiptImageReplace: "Replace",
  currentAccountReceiptCurrency: "Currency",
  ledgerHistoryTitle: "Receipt history",
  ledgerHistoryEmpty: "No receipts yet.",
  ledgerReceiptLinkedToInvoice: "Invoice #{id}",
  ledgerReceiptGeneral: "General payment",
  ledgerReceiptDelete: "Delete",
  ledgerReceiptDeleted: "Receipt deleted.",
  ledgerReceiptCreated: "Receipt saved.",
  ledgerModalTitle: "General receipt",
  ledgerModalDate: "Date",
  ledgerModalAmount: "Amount",
  ledgerModalKind: "Receipt kind",
  ledgerModalKindCash: "Cash",
  ledgerModalKindBankTransfer: "Bank transfer",
  ledgerModalKindCheck: "Check",
  ledgerModalKindPromo: "Promo/Discount",
  ledgerModalKindAdvance: "Advance payment",
  ledgerModalKindOther: "Other",
  ledgerModalNotes: "Notes (optional)",
  ledgerModalSave: "Save receipt",
  ledgerModalCancel: "Cancel",
  ledgerInvalidAmount: "Enter a valid amount.",
  currentAccountReceiptsHint:
    "View the receipts recorded against each invoice; edit or soft-delete as needed.",
  currentAccountReceiptsEmpty: "No receipts have been recorded for this branch.",
  currentAccountReceiptsInvoiceEmpty: "No receipts for this invoice yet.",
  currentAccountReceiptsInvoiceReceiptCount: "{n} receipt line(s)",
  currentAccountReceiptsTotalCount: "Total receipt lines",
  currentAccountReceiptsTotalAmount: "Receipts total",
  currentAccountReceiptsMixedCurrency: "(mixed currencies)",
  currentAccountReceiptsSearchPlaceholder: "Search by invoice no",
  currentAccountReceiptsFilterOnlyWithReceipts: "Only invoices with receipts",
  currentAccountReceiptKindLabel: "Receipt kind",
  currentAccountReceiptKindCash: "Cash / Bank",
  currentAccountReceiptKindPromo: "Promo / Discount",
  currentAccountReceiptKindAdvance: "Advance",
  currentAccountReceiptKindOther: "Other",
  currentAccountReceiptEdit: "Edit",
  currentAccountReceiptDelete: "Delete",
  currentAccountReceiptDeleteAsk: "Delete this receipt?",
  currentAccountReceiptDeleteConfirm: "Yes, delete",
  currentAccountReceiptDeleteCancel: "Cancel",
  currentAccountReceiptDeleted: "Receipt deleted.",
  currentAccountReceiptUpdateModalTitle: "Edit receipt",
  currentAccountReceiptSaveEdit: "Save changes",
  currentAccountReceiptUpdated: "Receipt updated.",
  currentAccountReceiptUpdateMaxAllowable: "Maximum allowable amount: {amount}",
  currentAccountReceiptOverpay:
    "Receipt amount cannot exceed the open balance. Maximum allowable amount: {amount}.",
  currentAccountReceiptDateInvalid: "Enter a valid date.",
  currentAccountPdfReceiptsSection: "Collections",
  currentAccountPdfRemaining: "Remaining",
  currentAccountPdfFileShipmentLabel: "Shipment",
  currentAccountPdfFileAccountLabel: "Current Account",
  currentAccountPdfChoiceTitle: "Choose PDF content",
  currentAccountPdfChoiceHint: "How should this document be generated?",
  currentAccountPdfChoiceContentLabel: "Content",
  currentAccountPdfChoiceV1: "Invoicing PDF (original)",
  currentAccountPdfChoiceV1Hint:
    "Downloads the original shipment PDF saved during invoicing; it is never modified.",
  currentAccountPdfChoiceV2: "With collections (v2)",
  currentAccountPdfChoiceV2Hint:
    "Adds the collections to the PDF and saves it separately as v2 on download. Subsequent downloads reuse the saved v2.",
  currentAccountPdfV2Saved: "Version with collections (v2) saved.",
  currentAccountPdfChoiceOriginal: "Original (plain)",
  currentAccountPdfChoiceOriginalHint:
    "The original shipment PDF; collections and remaining are not added.",
  currentAccountPdfChoiceWithReceipts: "Collections and remaining",
  currentAccountPdfChoiceWithReceiptsHint:
    "All collections are appended at the bottom and the remaining is shown; totals are preserved.",
  currentAccountPdfChoicePriorLabel: "Prior account balance",
  currentAccountPdfChoicePriorInclude: "Include (full account)",
  currentAccountPdfChoicePriorIncludeHint:
    "Open balance carried from earlier invoices is added to the net amount.",
  currentAccountPdfChoicePriorExclude: "Exclude (shipment only)",
  currentAccountPdfChoicePriorExcludeHint:
    "Only this shipment's amounts are shown.",
  tourismSeasonClosedOpenTab: "Go to tourism season tab",
  expensesSeasonMissingForToday:
    "There is no open tourism season covering today for this branch. Without an operating season, posting branch-register expenses is blocked on the server; add a season row or pick a transaction date inside an open season. Only register income lines may still be allowed when the central “Closed season allowances” policy permits it.",
  zReportTabFilterYear: "Year",
  zReportTabFilterYearPh: "Empty = current year",
  zReportTabResetYear: "Reset year",
  notesHint: "General branch notes as separate lines (e.g. contacts, reminders).",
  notesReadOnlyHint: "You can view notes; only staff can add or edit them.",
  notesAdd: "Add note",
  notesEmpty: "No notes for this branch yet.",
  notesEdit: "Edit",
  notesDelete: "Delete",
  notesDeleteAsk: "Delete this note?",
  notesDeleteConfirm: "Yes, delete",
  notesAddTitle: "Add note",
  notesEditTitle: "Edit note",
  notesBodyLabel: "Note text",
  notesBodyRequired: "Note text is required.",
  notesBodyTooLong: "Note text cannot exceed 4000 characters.",
  docKindTaxBase: "Tax base certificate",
  docKindWorkPermit: "Work permit / operating certificate",
  docKindAgricultureCert: "Agriculture / related certificate",
  docKindShipmentDeliverySlip: "Shipment delivery slip",
  docKindOther: "Other",
  documentsGroupShipmentStatement: "Shipment / account PDFs",
  documentsTabSystem: "System PDFs",
  documentsTabUploaded: "Uploaded documents",
  documentsFilterAll: "All",
  documentsBadgeOriginal: "Original (v1)",
  documentsBadgeWithReceipts: "With collections (v2)",
  documentsIntro:
    "Upload PDF or image files (tax base, work permit, agriculture certificate, etc.). Staff can add and remove; everyone who can open the branch can download.",
  documentsAdd: "Upload document",
  documentsEmpty: "No documents uploaded for this branch yet.",
  documentsView: "View",
  documentsShipmentNo: "Shipment No",
  documentsStartAction: "Start action",
  documentsUploadTitle: "Upload branch document",
  documentsKindLabel: "Document type",
  documentsFileLabel: "File (PDF or image)",
  documentsFileRequired: "Choose a file to upload.",
  documentsNotesLabel: "Notes (optional)",
  documentsNotesPlaceholder: "e.g. expiry date, issuing authority",
  documentsUploadSubmit: "Upload",
  documentsDeleteTitle: "Delete document",
  documentsDeleteConfirm: "Delete this file? This cannot be undone.",
  tSeasonHint:
    "Record season year and open/close dates retrospectively. If close date is empty, the season is treated as still open.",
  tSeasonGuideTitle: "Tourism season & closure — how it works",
  tSeasonGuideStep1:
    "For each season year, set the open date. When the season ends, set the close date; if close is empty, the season is still treated as open.",
  tSeasonGuideStep2:
    "When adding a new season year, the system checks prior-calendar-year account closure for personnel tied to this branch; if incomplete, it lists them and blocks save.",
  tSeasonGuideStep3:
    "Income and expense summaries use the season row that covers “today” based on the dates you maintain here.",
  tSeasonGuideStep4:
    "If the tourism season is closed on a transaction day, register operation types follow the central “Allowed when season closed” policy.",
  tSeasonGuidePolicyLink: "Manage closed-season allowances (admins)",
  tSeasonGuidePolicyNoteViewer:
    "Which register actions stay open when the season is closed is a central setting; ask your system administrator to change it.",
  tSeasonFilterYear: "Season year (filter)",
  tSeasonFilterYearPh: "Empty = all years",
  tSeasonFilterYearInvalid: "Enter a year between 1990 and 2100, or clear the field.",
  tSeasonFilterAllYears: "All years",
  tSeasonAdd: "Add record",
  tSeasonEmpty: "No tourism season records for this branch yet.",
  tSeasonEdit: "Edit",
  tSeasonDelete: "Delete",
  tSeasonDeleteAsk: "Delete this record?",
  tSeasonDeleteConfirm: "Yes, delete",
  tSeasonStillOpen: "Open (no close date)",
  tSeasonColYear: "Season year",
  tSeasonColOpened: "Opened",
  tSeasonColClosed: "Closed",
  tSeasonColClosedOptional: "Closed (optional)",
  tSeasonColNotes: "Notes",
  tSeasonColCreated: "Created",
  tSeasonAddTitle: "Tourism season record",
  tSeasonEditTitle: "Edit record",
  tSeasonFormHint:
    "e.g. 2026 season opened 15 March; leave closed empty if not known yet.",
  tSeasonYearInvalid: "Season year must be between 1990 and 2100.",
  tSeasonOpenedInvalid: "Open date is required.",
  tSeasonClosedBeforeOpen: "Close date cannot be before open date.",
  tSeasonYearClosureGateLoading: "Checking prior-year personnel account status…",
  tSeasonYearClosureGateError:
    "Could not verify account-closure rules. Retry, or save anyway — the server will reject if rules are not met.",
  tSeasonYearClosureBlockedTitle: "Prior calendar year not closed for some personnel",
  tSeasonYearClosurePriorLabel: "Prior calendar year",
  tSeasonYearClosureBlockedListIntro:
    "Close their year from Personnel → Period account summary before adding this tourism season.",
  zReportAccountingTitle: "Z report — accounting (monthly)",
  zReportAccountingHint:
    "Only months that overlap a tourism season row are tracked. Confirmation is stored with date and user.",
  zReportColMonth: "Month",
  zReportColStatus: "Status",
  zReportStatusNa: "No tourism season",
  zReportStatusUpcoming: "Upcoming",
  zReportStatusPending: "Not sent",
  zReportStatusSent: "Sent",
  zReportColSentAt: "Confirmed at",
  zReportColSentBy: "By",
  incomeSummarySectionTitle: "Branch income — summary",
  incomeSummaryCardsLead:
    "Top: all-time and (when defined for today) open tourism season income through today. The cards below follow the list dates in Filters; they do not change the top cumulatives. Cash cards include who holds the cash.",
  incomePatronKasaSplitTitle: "Patron vs register (same calendar day)",
  incomePatronKasaSplitLead:
    "Uses the register-summary date: off-register advances (no register expense yet), OUT_NON_PNL by payment source, other expenses by source, and net owed to the owner for that day.",
  incomeSplitOffRegisterAdvTitle: "Off-register advances (recorded that day)",
  incomeSplitOffRegisterAdvPatron: "Patron-funded (PATRON)",
  incomeSplitOffRegisterAdvBank: "Bank-funded (BANK)",
  incomeSplitOffRegisterAdvTotal: "Total (accounting advance line)",
  incomeSplitOutNonPnlTitle: "OUT_NON_PNL lines — payment source",
  incomeSplitOutNonPnlRegister: "From register cash",
  incomeSplitOutNonPnlPatron: "Patron-paid",
  incomeSplitOutNonPnlPocket: "Staff pocket",
  incomeSplitOutNonPnlTotal: "Total OUT_NON_PNL",
  incomeSplitOtherOutTitle: "Other economic expenses (excl. OUT_NON_PNL)",
  incomeSplitOtherOutRegister: "Register cash",
  incomeSplitOtherOutPatron: "Patron-paid",
  incomeSplitOtherOutPocket: "Staff pocket",
  incomeSplitDayNetPatron: "Net owed to owner (day)",
  incomeCumulativeAllTimeTitle: "All time — through today",
  incomeCumulativeSeasonTitle: "This season — through today",
  incomeSeasonYearShort: "Season year",
  incomeSeasonOpenEnded: "open season",
  incomeSeasonMissingForToday: "No tourism period covers today for this branch; add one under Tourism season.",
  incomeListDatesSummaryTitle: "Summary for list dates",
  incomeListDatesSummaryLead:
    "Set start and end under Filters. Same day = that day’s register snapshot; different days = period total. Empty both dates defaults to today. Amounts transferred to the owner appear inside the cards for the same date scope as the list.",
  incomeListPatronTransferredCash: "Cash transferred to owner (filter)",
  incomeListPatronTransferredCard: "Card/POS to owner (filter)",
  incomeListPatronTransferredTotal: "Income to owner (filter)",
  incomeListFilterScopeHint: "Filter scope",
  incomeListUnifiedBreakdownHint:
    "Split inside this total (amount and percentage), calculated by filters.",
  incomeListPosPatronHint:
    "Card/POS intake is tracked on the owner side per branch POS settlement policy; see cash settlement on line items.",
  incomeListInvalidRange: "Start date cannot be after end date.",
  incomeListDatesIncomplete: "Dates are incomplete or invalid; both fields need a full date.",
  incomeCashPartyPatron: "Cash — patron",
  incomeCashPartyBranchManager: "Cash — branch lead",
  incomeCashBranchManagerBreakdownToggle: "Split by named cashier (details)",
  incomeCashBranchManagerUnassigned: "Recorded without named cashier",
  incomeCashPartyRemains: "Cash — stays at branch",
  incomeCashPartyUnspecified: "Cash — unset / other",
  incomeDayForDatePrefix: "Day summary —",
  incomePeriodTitle: "Period summary (income)",
  incomePeriodForRangePrefix: "Selected range —",
  incomePeriodTotal: "Period — income",
  incomePeriodCash: "Period — cash",
  incomePeriodCashEarned: "Earned",
  incomePeriodCashInHand: "In hand",
  incomePeriodCashSpent: "Spent from register (intraday)",
  incomeCashDistTitle: "Cash in hand — where it went",
  incomeCashDistPersonnel: "To staff",
  incomeCashDistPatron: "To owner",
  incomeCashDistRemains: "Stays in register",
  incomeCashDistUnset: "Unassigned",
  incomePeriodCard: "Period — credit card (POS)",
  incomeCloseTitle: "Day summary (income)",
  incomeCloseTotal: "Day — income",
  incomeCloseCash: "Day — cash",
  incomeCloseCard: "Day — credit card (POS)",
  incomeCumulativeTitle: "Through today (all time)",
  incomeCumulativeTotal: "Cumulative — income",
  incomeCumulativeCash: "Cumulative — cash",
  incomeCumulativeCard: "Cumulative — credit card (POS)",
  incomeActionsTitle: "Income actions",
  incomeFilterOpenButton: "Filters",
  incomeFilterDrawerTitle: "Income list filters",
  incomeFilterDrawerHint:
    "These dates filter the income list only. Top cumulatives are always through today; day / range cards live in the section below.",
  incomeFilterApplyAndClose: "Apply & close",
  incomeListSection: "Income lines",
  incomeQuickFiltersLead: "Quick dates & refresh list",
  noIncome: "No income matches these filters.",
  noIncomeTitle: "No income matches these filters",
  noIncomeHint: "Widen the date range, try «All dates», or adjust filters.",
  noIncomeClearFilters: "Clear all filters",
  mobileJumpIncomeNavAria: "Jump to a section on the income tab",
  mobileJumpIncomeSummary: "Cumulative",
  mobileJumpIncomePatron: "Owner / register",
  mobileJumpIncomeListDates: "List date",
  mobileJumpIncomeLines: "Income list",
  mobileJumpExpenseNavAria: "Jump to a section on the expenses tab",
  mobileJumpExpenseSummary: "Expense summary",
  mobileJumpExpenseListDates: "List date",
  mobileJumpExpenseLines: "Expense list",
  dashMonthPicker: "Report month",
  dashPersonnel: "Staff at branch",
  dashMonthIncome: "Monthly income",
  dashMonthExpense: "Monthly expense",
  dashMonthExpenseBreakdownTitle: "Expense breakdown",
  dashMonthExpenseBreakdownHint:
    "Fixed and variable operating expense (OPEX) plus investment/equipment (CAPEX). Personnel and other items are outside these buckets.",
  dashMonthExpenseFixed: "Fixed",
  dashMonthExpenseVariable: "Variable",
  dashMonthExpenseCapex: "Investment / equipment",
  dashMonthExpenseOther: "Personnel & other",
  dashMonthExpenseOtherHint: "Monthly expense − (fixed + variable + investment): personnel and behavior-less items.",
  dashMonthExpenseFamilyOps: "Operations",
  dashMonthExpenseFamilyCapex: "Investment",
  dashAllIncome: "All-time income",
  dashAllExpense: "All-time expense",
  dashCashRegister: "Cash in register (through today)",
  dashTodayIncome: "Today income",
  dashTodayExpense: "Today expense",
  dashOpsOverviewTitle: "Operations overview",
  dashInsuredPersonnel: "Insured personnel",
  dashInsuredPersonnelHint: "Count of personnel with an open insurance period.",
  dashExpenseSourceBreakdownTitle: "Expense payment-source breakdown",
  dashExpenseSourceBreakdownHint: "Selected-day split by owner/register/staff pocket.",
  dashSelectedDayExpenseTotal: "Selected-day expense total",
  dashHeldRegisterCashTitle: "Register cash held with personnel",
  dashHeldRegisterCashTotal: "Total open held cash",
  dashHeldRegisterCashTotalHint:
    "Net amount handed over to personnel through the selected date and not returned to register yet.",
  dashHeldRegisterCashTopHolder: "Highest open holder",
  dashEntryDayCount: "Transaction day count",
  dashEntryDayCountHint: "Distinct dates visible in the selected-day list only.",
  dashSelectedDayInCount: "Income lines",
  dashSelectedDayOutCount: "Expense lines",
  dashDayMovements: "Register lines for selected day",
  registerDayBookTitle: "Income and expense lines recorded for this day",
  expensesHint:
    "Top: all-time through today, then (when applicable) this tourism season through today with the same cards. «Summary for list dates» defaults to today. Expenses do not use a «cash with branch cashier» split.",
  expensesSummarySectionTitle: "Branch expenses — summary",
  expensesSummaryCardsLead:
    "Top: all-time through today and (when applicable) the tourism season through today with the same expense cards. Cards are not mutually exclusive; explanatory notes appear in each block.",
  expensesCloseTitle: "Day-close register summary",
  expensesCloseHint:
    "From and to are the same day — cards and list use that date. «Net owed to owner» subtracts same-day register repayments to the owner; the violet box above (when shown) is gross owner-paid expenses for the list filters.",
  expensesCloseIncome: "Income (day)",
  expensesCloseExpense: "Expense, accounting (day)",
  expensesCloseNet: "Net (day)",
  expensesCloseCashOut: "Out from register (day)",
  expensesClosePatronPaid: "Net owed to owner (day)",
  expensesCloseNonRegister: "Off-register advance (day)",
  expensesClosePickSingleDay:
    "Pick the same start and end date for the summary, or tap Today.",
  expensesSummaryLifetimeBlockTitle: "All time (through today)",
  expensesSummaryLifetimeBlockLead:
    "From the branch’s first recorded movement through today: takings plus how expenses were paid and grouped by type.",
  expensesSummarySeasonBlockTitle: "This tourism season (through today)",
  expensesSummarySeasonBlockLead:
    "From the active season’s first day through today using the same measures; compare with «all time» above.",
  expensesTabTotalIncome: "Total takings",
  expensesTabOutRegister: "Paid from the branch register",
  expensesTabOutPatron: "Paid by the owner",
  expensesTabOutPocket: "Paid from staff pocket",
  expensesTabOutPocketHint: "No cash left the drawer; amount reflects register debt to staff.",
  expensesTabPersonnelMain: "Direct staff-related expenses",
  expensesCostBehaviorTitle: "Cost behavior",
  expensesCostBehaviorLead: "Operating expense (fixed + variable) vs investment",
  expensesCostBehaviorFixed: "Fixed cost",
  expensesCostBehaviorFixedHint:
    "Costs you pay roughly the same every period regardless of sales/volume and cannot avoid: rent, utilities, insurance, software, security, license, bank, franchise. They continue even if the branch is closed.",
  expensesCostBehaviorVariable: "Variable cost",
  expensesCostBehaviorVariableHint:
    "Costs that rise and fall with activity/turnover: fuel, cargo, meals, cleaning, marketing, ad-hoc repairs, goods purchases. No activity, no spend.",
  expensesCostBehaviorOpexTotal: "Operating expense (total)",
  expensesCostBehaviorOpexTotalHint: "Fixed + variable; the total operating cost of running the branch.",
  expensesCostBehaviorCapex: "Investment / fixtures",
  expensesCostBehaviorCapexHint:
    "Capital/fixture purchases (AC, camera, furniture, fit-out). NOT included in the operating expense total; shown separately.",
  expensesTabBranchOps: "Branch operating expenses",
  expensesTabBranchOpsHint:
    "Goods/stock, operations, tax, and other expense mains (OUT_GOODS, OUT_OPS, OUT_TAX, OUT_OTHER). Staff, advances, and general-overhead share are separate cards.",
  expensesTabAdvanceNonPnl: "Advances and similar off-register items",
  expensesTabGeneralOverhead: "Share from general overhead",
  expensesTabGeneralOverheadHint:
    "Total allocated to this branch from central general-overhead pools. If paid by owner/register, the same amount also appears on those cards.",
  expensesSummaryCardsOrthogonalNote:
    "Cards are not mutually exclusive: one expense line can appear on several cards at once (payment source, main category, and general-overhead share). Do not add card totals to invent a single “total expense”.",
  expensesOverlapTitle: "General-overhead share — the same expense line also shows up as:",
  expensesOverlapBulPatron: "Amount also counted on the owner-paid card:",
  expensesOverlapBulRegister: "Amount also counted on the register-paid card (cash component rules):",
  expensesOverlapBulPocket: "Amount also counted on the staff-pocket card:",
  expensesOverlapBulBranchMain: "Amount also counted on the branch operating bucket:",
  expensesOverlapFooter:
    "These slices refer to the same underlying rows; they answer different questions. Use the table below or the expense list for the net line view.",
  expensesCardDetailOverheadOverlapIntro:
    "Each row is one expense transaction. The same amount may also appear on owner/register/operating cards above—do not sum cards across dimensions.",
  expensesCardDetailRegisterGoOverlapLabel: "Including general-overhead share (register):",
  expensesCardDetailRegisterGoOverlapExplain:
    "Central allocation paid/treated from the register; can be the same movement as the general-overhead card.",
  expensesCardDetailPatronGoOverlapLabel: "Including general-overhead share (owner-paid):",
  expensesCardDetailPatronGoOverlapExplain:
    "Central allocation marked owner-paid; can overlap with the general-overhead and operating cards.",
  expensesCardDetailPocketGoOverlapLabel: "Including general-overhead share (staff pocket):",
  expensesCardDetailPocketGoOverlapExplain:
    "Central allocation marked staff-pocket paid; can be the same line as the general-overhead card.",
  expensesCardDetailBranchGoOverlapLabel: "Including general-overhead share (operating mains):",
  expensesCardDetailBranchGoOverlapExplain:
    "If the allocated line’s main category is goods/ops/tax/other, it also contributes here alongside general-overhead and payment-source views.",
  expensesCardAxisIncome: "Income; not a payment-source slice.",
  expensesCardAxisRegister: "Slice: paid from the branch register.",
  expensesCardAxisPatron: "Slice: paid by the owner (outside the register).",
  expensesCardAxisPocket: "Slice: paid from staff pocket.",
  expensesCardAxisMainCategoryPersonnel:
    "Slice: main category (staff). Register vs owner: use the register/owner/pocket cards and each line.",
  expensesCardAxisMainCategoryBranch:
    "Slice: operating mains. Register vs owner: use the register/owner/pocket cards and each line.",
  expensesCardAxisMainCategoryAdvance:
    "Slice: advance / off-register mains. Payment: line payment source + cards above.",
  expensesCardAxisCentralShare:
    "Slice: central general-overhead allocation. The same amount may also appear on register/owner/pocket cards.",
  expensesCardAxisTopMain: "Slice: largest main category. Payment is separate (register/owner/pocket cards).",
  expensesCardAxisOutLines: "Slice: economic expense line count. Payment is separate.",
  expensesCardTapForDetail: "Tap for details",
  expensesTabTopExpenseCategory: "Top expense main category",
  expensesTabTopExpenseCategoryEmpty: "—",
  expensesTabEconomicOutCount: "Economic expense line count",
  expensesTabEconomicOutCountHint:
    "Number of expense lines in the period, excluding advances and debt-repayment lines from the economic bucket.",
  expensesCardDetailClose: "Close",
  expensesCardDetailIncome:
    "Sum of all income lines in this period. Use the Income tab to review individual lines.",
  expensesCardDetailRegister:
    "Expenses paid from the branch register: register payment source (or legacy blank) with the same cash-split rules as summaries.",
  expensesCardDetailPatron:
    "Expenses paid directly by the owner (owner-paid). Reflects what the branch register owes the owner for those lines.",
  expensesCardDetailPocket:
    "Expenses paid from staff pocket. No cash left the drawer; amounts increase the register’s debt to the staff member.",
  expensesCardDetailPersonnel:
    "Total expense lines whose main category is staff-related (depends on how each line was categorized).",
  expensesCardDetailBranch:
    "Sum of expense lines tagged for day‑to‑day branch operations: goods/stock (OUT_GOODS), operations (OUT_OPS), tax (OUT_TAX), and other expense (OUT_OTHER). Staff (OUT_PERSONNEL), off‑register/advance (OUT_NON_PNL), and debt‑repayment mains are excluded. If a central general‑overhead allocation line uses one of those four mains, it is included here too and may also appear on the general‑overhead share card.",
  expensesCardDetailMatchingCardTotalLabel: "Amount on the card (this period)",
  expensesCardDetailGoTableVsCardHint:
    "The table below lists only central general‑overhead allocation lines. The card total also includes non‑pool branch lines in the same mains, so the table does not have to add up to the card.",
  expensesCardDetailBranchNonGoPortionLabel:
    "Branch operating expenses without a general‑overhead pool (card total minus the general‑overhead share above):",
  expensesCardDetailBranchNonGoPortionHint:
    "These are branch expense lines in OUT_GOODS / OUT_OPS / OUT_TAX / OUT_OTHER that are not central pool allocations. Filter the expense list by those mains for line detail.",
  expensesBranchOperatingListTitle: "Lines in this period",
  expensesBranchOperatingListLead:
    "Badges: purple = central general‑overhead share; green = direct branch (same mains, no pool). The card total matches the sum of these rows (within the row cap).",
  expensesBranchLineBadgeGo: "General‑overhead share",
  expensesBranchLineBadgeDirect: "Direct branch",
  expensesBranchOperatingListTruncated:
    "Only the latest 250 lines are shown; use the expense list for older rows.",
  expensesBranchOperatingListEmpty: "No lines to list for this period.",
  expensesCardDetailAdvance:
    "Total of OUT_NON_PNL style lines tracked as advances / off-register items in this period.",
  expensesCardDetailOverheadEmpty: "No general-overhead allocation lines in this period.",
  expensesCardDetailGoColPool: "Pool",
  expensesCardDetailGoColDate: "Date",
  expensesCardDetailGoColAmount: "Amount",
  expensesCardDetailGoColMain: "Main category",
  expensesCardDetailGoColPay: "Payment",
  expensesCardDetailGoColDesc: "Description",
  expensesCardDetailGoColTx: "Line id",
  expensesCardDetailGoLinesTitlePatron: "General-overhead lines — owner-paid",
  expensesCardDetailGoLinesTitleRegister: "General-overhead lines — counted toward register share",
  expensesCardDetailGoLinesTitlePocket: "General-overhead lines — staff pocket",
  expensesCardDetailGoLinesTitleBranch: "General-overhead lines — operating main categories",
  expensesCardDetailGoLinesFilteredEmpty: "No general-overhead lines match this slice.",
  expensesCardDetailGoLinesRegisterFootnote:
    "Rows use the same filter as the register share figure (e.g. unpaid ops invoices excluded from the register component). Cash/card splits can make line totals differ from the headline amount.",
  expensesCardDetailTopCategoryEmpty: "No economic expenses in this period or no category split.",
  expensesCardDetailTopCategoryLead:
    "The main expense category with the largest summed amount in the period (same economic filters as the day-close style summaries).",
  expensesCardDetailOutCountLead:
    "Count of distinct economic expense transactions in the same date range.",
  expensesListDatesSummaryTitle: "Summary for list dates",
  expensesListDatesSummaryLead:
    "Set start and end under «Filters». Same day: that day’s register bridge; a range: filtered expense total, owner-paid expense total, and row count. Empty dates default to today.",
  expensesListFilteredTotal: "Filtered expense total",
  expenseAddOps: "Operating Expense",
  expenseAddOpsHint: "Rent, utilities, maintenance, fuel — paid from register",
  expenseAddSupplier: "Supplier Invoice",
  expenseAddSupplierHint: "Stocked/non-stocked invoice with payment plan",
  expenseAddOverhead: "General Overhead Pool",
  expenseAddOverheadHint: "Shared expense allocated across branches",
  expenseAddMoreOptions: "More expense types",
  expenseAddPickTitle: "What kind of expense?",
  txCashHandoverSummaryTitle: "Cash handover summary",
  txCashHandoverEnteredCash: "Cash income entered:",
  txCashHandoverPriorExpenses: "Day expenses from register:",
  txCashHandoverBundledExpenses: "Newly added register expenses:",
  txCashHandoverNetTransfer: "Net cash to transfer:",
  txCashHandoverHint: "Register expenses are automatically deducted from the entered cash income before transfer.",
  expensesListFilterScopeHint: "Filter scope",
  expensesListUnifiedBreakdownHint:
    "Payment-source split within this total (amount and percentage).",
  expensesListExcludesHeldRegisterHint:
    "Held register cash handovers are not counted as expenses—only register, owner, and personnel-pocket payments. Filter by «Personnel-held register cash» to list those transfers.",
  expensesListBreakdownHint:
    "Total amount split by payment source (register cash, owner, personnel-held).",
  expensesAxisCashOut: "Paid from register (cash out)",
  expensesAxisOffRegister: "Off-register (owner / personnel pocket)",
  expensesAxisSplitHint:
    "Register payments reduce drawer cash; off-register items (e.g. owner-paid invoices) are accounting expenses but do not affect the register or day-close.",
  expensesOffRegisterPatronBadge: "Off-register · Owner paid",
  expensesListPeriodPatronTotal: "Owner-paid expense total",
  expensesListPeriodRowCount: "Matching rows",
  expensesListDayForPrefix: "Selected day:",
  expensesActionsTitle: "Expense actions",
  expenseFilterDrawerTitle: "Expense list filters",
  expenseFilterDrawerHint:
    "Date and category filters match the list below. Apply refreshes the list and closes the drawer.",
  expenseFilterApplyAndClose: "Apply and close",
  expensesListSection: "Expense lines",
  expenseQuickFiltersLead: "Quick dates & refresh list",
  expenseHeldSupplierPaymentsTitle: "Supplier payments (from personnel-held cash)",
  expenseHeldSupplierPaymentsHint:
    "These payments were made from cash held by personnel. The expense is counted on the supplier side; it is not included in the expense totals (KPIs) here — shown for information only.",
  expenseHeldSupplierFallback: "Supplier payment",
  expenseHeldSupplierAllPersonnel: "All personnel",
  expenseHeldPersonnelFilterLabel: "Held-cash personnel",
  expenseHeldSupplierAllSuppliers: "All suppliers",
  expenseHeldSupplierSortDate: "By date",
  expenseHeldSupplierSortAmount: "By amount",
  expenseHeldSupplierSubtotal: "Subtotal",
  noExpensesTitle: "No expenses match these filters",
  noExpensesHint: "Widen the date range, try «All dates», or adjust filters.",
  noExpensesClearFilters: "Clear all filters",
  filterToday: "Today",
  filterAllDates: "All dates",
  filterAllTime: "All time",
  filterThisSeason: "This season",
  stockSingleDayBanner:
    "Selected day {date}: only warehouse receipts on this day are listed below.",
  noExpenses: "No expenses match these filters.",
  stockHint:
    "Inbound stock from warehouses to this branch, grouped by shipment/batch like warehouse movements. Category/product and date filters match the summary card.",
  stockInboundSectionTitle: "Inbound stock to branch",
  stockQuickFiltersLead: "Quick dates & refresh list",
  stockScopeFiltersLead: "Product & category scope",
  stockDateFiltersLead: "Date range",
  stockFiltersSummaryLead: "Active filter summary",
  stockFiltersScopeActiveShort: "Product scope narrowed",
  stockFiltersScopeAnyShort: "All products in scope",
  stockFiltersDrawerTitle: "Product & date filters",
  stockFiltersDrawerOpenButton: "Product & date filters",
  stockFiltersDrawerHint:
    "Selections match the list and summary card. Apply refreshes data and closes the panel.",
  stockFiltersApplyClose: "Apply and close",
  stockFiltersResetInDrawer: "Clear filters",
  stockListViewModeHint: "Choose how rows on this page are grouped.",
  stockListViewModeAria: "Stock receipt list grouping",
  stockListViewShipment: "Shipment group",
  stockListViewMainProduct: "Main product",
  stockReceiptsPageMainProductSummary: "This page: {{groups}} main-product group(s) · {{lines}} line(s)",
  stockDetailModalTitleMainProduct: "Main product — lines",
  stockDetailModalHintMainProduct: "Group: {{name}}",
  stockEmptyTitle: "No stock receipts match these filters",
  stockEmptyHint: "Widen the date range, try «All dates», or reset product scope.",
  stockEmptyClearFilters: "Clear dates & product filters",
  noStockReceipts: "No receipts found.",
  stockColDate: "Date",
  stockColProduct: "Product",
  stockColQty: "Qty",
  stockColWarehouse: "Source warehouse",
  stockColCreatedBy: "Entered by",
  stockColCreatedAt: "Created at",
  stockCreatedByUnknown: "Unknown",
  stockGroupRecordMeta: "Entered by: {{user}} · {{date}}",
  stockReceiptsTotalsTitle: "Summary (matches filters)",
  stockReceiptsFilteredTotalQty: "Total inbound quantity",
  stockReceiptsParentBreakdownTitle: "Main product breakdown",
  stockReceiptsParentBreakdownHint: "Main product total shown here: {{qty}}",
  stockReceiptsParentBreakdownChildrenToggle: "Toggle sub-product breakdown for {{name}}",
  stockReceiptsParentBreakdownPageOnly:
    "Note: This breakdown is calculated from the current page only; the filter total covers all pages.",
  stockReceiptsTotalsHint:
    "Sum of branch stock-in quantities for the selected date range and product scope (all pages).",
  stockReceiptsPageShipmentSummary: "This page: {{shipments}} shipment group(s) · {{lines}} line(s)",
  stockShipmentQuickOpen: "Details",
  stockShipmentModalTitle: "Shipment details",
  stockShipmentModalHint: "Shipment: {{shipment}}",
  stockShipmentModalHintLead: "Shipment",
  stockShipmentModalViewHint: "Choose how to display line items.",
  stockShipmentModalViewAria: "Shipment detail view",
  stockShipmentModalViewLines: "By line",
  stockShipmentModalViewMainProduct: "By main product",
  stockShipmentModalGroupLineCount: "{{count}} line(s)",
  stockShipmentModalOpenWarehouse: "Warehouse detail",
  stockShipmentPhotoHeading: "Shipment photo",
  stockShipmentPhotoNone: "No photo attached for this shipment.",
  stockShipmentPhotoRemove: "Remove photo",
  stockShipmentPhotoReplaceHint: "Selecting a new photo replaces the current one.",
  stockShipmentPhotoAddHint: "Attach a photo for this shipment (JPEG/PNG/WebP).",
  stockShipmentPhotoSaved: "Shipment photo saved.",
  stockShipmentPhotoRemoved: "Shipment photo removed.",
  filterDateFrom: "From",
  filterDateTo: "To",
  filterApplyRefresh: "Refresh",
  txFilterAny: "All",
  txFilterMainCategory: "Main category",
  txFilterSubCategory: "Sub-category",
  txFilterCategory: "Category",
  txFilterCashSettlement: "Physical cash handover",
  txFilterExpensePayment: "Payment source",
  txFilterExpenseKind: "Expense kind",
  expKindOps: "Operating expense",
  expKindPersonnel: "Personnel (advance / salary)",
  expKindSupplier: "Supplier invoice",
  expKindVehicle: "Vehicle maintenance / fuel",
  expKindContractor: "Contractor / subcontractor",
  branchPdfExpenseGroupPersonnel: "Personnel held cash",
  expensePayRegister: "Branch register",
  expensePayPatron: "Owner's pocket",
  expensePayPersonnelPocket: "Staff pocket",
  expensePayPersonnelHeldRegisterCash: "Register cash held by personnel",
  expensePayHintRegister: "Posted to branch register.",
  expensePayHintPatron: "Not from branch register; owner pays.",
  expensePayHintPersonnelPocket: "Register owes the staff; no register cash out.",
  expensePayHintPersonnelHeldRegisterCash: "Deducted from staff's held register cash.",
  expensePayRegisterShort: "Register",
  expensePayPatronShort: "Owner",
  expensePayPersonnelPocketShort: "Custody",
  expensePayPersonnelHeldRegisterCashShort: "Custody",
  expensePocketRepayViaRegister: "Register → staff",
  expensePocketRepayViaPatron: "Owner → staff",
  expensePocketPersonLabel: "Staff member who paid",
  expenseHeldRegisterPersonLabel: "Staff member holding the register cash",
  expenseHeldRegisterPersonPickerLoading: "Loading held register cash balances…",
  expenseHeldRegisterPersonPickerEmpty:
    "No staff with a positive held register cash balance through this date.",
  expenseHeldRegisterPersonPickerNeedDate: "Set a valid transaction date to load balances.",
  /** Held register dropdown: label before formatted amount. */
  heldRegisterCashDropdownAmountLabel: "Cash on hand",
  expensePocketRepayPersonLabel: "Staff member receiving payment",
  pocketPriorLinesTitleRepay: "Staff-pocket expense lines to settle",
  pocketPriorLinesHintRepay:
    "Required for «Pocket debt repayment»: the repayment amount is only the sum of lines you select here; each line can be settled once (unsettled lines are listed).",
  pocketRepayPickRequired: "Select at least one staff-pocket expense line.",
  pocketRepayAmountMismatch: "Amount must equal the sum of the selected lines.",
  pocketRepayAmountFromSelectionHint:
    "Amount is set only from the list below; you cannot type it manually.",
  pocketPriorSelectAll: "Select all",
  pocketPriorClear: "Clear selection",
  pocketPriorSelectedTotal: "Selected total",
  pocketPriorEmpty: "No staff-pocket expense lines for this person yet.",
  pocketPriorCurrencyMismatch:
    "Selected lines use a different currency than the form. Match the currency to auto-fill the amount.",
  txExpensePocketPersonnelRequired:
    "When staff pocket is selected, choose who paid (the register owes that person).",
  expensePaymentLabel: "How was this paid?",
  expensePaymentUnset: "Not specified",
  expensePaymentHint:
    "Mark whether cash left the register, the owner covered it, staff paid from pocket (register debt), or it was paid from register cash already held with a staff member. The \"held register cash\" option is not owner-funded; no new drawer cash leaves the register, and the amount is reduced from that staff-held register balance.",
  txPersonnelExpenseBranchLabel: "Branch (for register payment)",
  txPersonnelExpenseBranchPick: "Select branch…",
  txPersonnelExpenseBranchHint:
    "Register payments post to this branch’s cash. For staff without an assigned branch, pick the branch first.",
  txContextBranchLabel: "Transaction branch",
  txExpenseTargetBranchLabel: "Branch this expense posts to",
  txExpenseTargetBranchHintEditable:
    "Choose which branch register this record will be posted to.",
  txExpenseTargetBranchHintFixed:
    "This record is fixed to this branch register.",
  txExpenseTargetBranchUnknown: "Branch not found",
  txPersonnelExpensePaymentHintNoBranch:
    "Pick the branch first, then mark register or owner payment.",
  txPersonnelExpensePaymentHintWithBranch:
    "Mark whether it was paid from the register or by the owner.",
  txRegisterPaymentNeedBranch: "Register payment requires a valid branch.",
  txSettlesCashHandoverLabel: "Apply against register physical cash handover income # (optional)",
  txSettlesCashHandoverHint:
    "When this expense reduces cash from a specific «cash to manager» income line, enter that line’s transaction id (from the personnel physical cash handover list). Leave blank if not applicable.",
  txSettlesCashHandoverInvalid: "Physical cash handover line id must be a positive number.",
  txSettlesCashHandoverNotAllowed:
    "Physical cash handover line id is not allowed for this expense type or payment source.",
  txAdvanceNeedBranch: "Register advance requires a valid branch.",
  receiptPhotoOptional:
    "Receipt photo (optional, max 8 MB — JPEG, PNG, WebP, HEIC/AVIF from phone)",
  receiptPhotoRequiredWhenInvoice:
    "Invoice image (required, max 8 MB — JPEG, PNG, WebP, HEIC/AVIF from phone)",
  invoiceReceiptPhotoRequired: "Supplier invoices require an invoice/receipt photo.",
  openReceiptPhoto: "Open receipt",
  txColActions: "Action",
  txDetailTitleIncome: "Income line details",
  txDetailTitleExpense: "Expense line details",
  txDetailViewAria: "View details",
  txDetailSectionGeneral: "General",
  txDetailSectionAmounts: "Amounts",
  txDetailSectionPayment: "Payment & cash",
  txDetailSectionLinks: "Linked records",
  txDetailSectionNote: "Note",
  txDetailGrossCash: "Gross cash earned",
  txDetailCardAmount: "POS / card",
  txDetailCashAmount: "Cash into register",
  txDetailNetIncome: "Net income",
  txDetailCashExpense: "Cash expense",
  txDetailNoNote: "No note entered",
  txDeleteAria: "Delete this register line",
  txDeleteSure: "Delete this entry?",
  txDeleteConfirmMessage:
    "The row will be soft-deleted; use «Undo» within 5 seconds to restore.",
  txDeleteIncomeToastMessage:
    "Are you sure you want to delete this income line? It will be soft-deleted (recoverable by support if needed).",
  txDeleteCancel: "Cancel",
  txDeleteConfirm: "Delete",
  txColReceipt: "Receipt",
  pagingTotal: "Total:",
  pagingPrev: "Previous",
  pagingNext: "Next",
  txDate: "Day",
  refreshTx: "Refresh",
  addTx: "Add income or expense",
  addIncomeTx: "Add income",
  addExpenseTx: "Add expense",
  patronFlowIncomeTitle: "Income paid out to owner (register)",
  patronFlowIncomeCardsSection: "Summary cards",
  patronFlowIncomeTotal: "Owner — income",
  patronFlowIncomeCash: "Owner — cash",
  patronFlowIncomeCard: "Owner — credit card (POS)",
  patronFlowIncomeUnspecified: "Cash vs card not split",
  patronFlowExpenseTitle: "Owner-paid expenses",
  patronFlowExpenseHint:
    "Total of expense lines with payment source «Owner», for the same filter range as the list.",
  dayIncome: "Income (selected day)",
  dayExpense: "Expense (selected day)",
  dayExpenseAccounting: "Expense, accounting (selected day)",
  dayCashOutDetail: "Out from register",
  dayNonRegisterAdvancesDetail: "Bank / patron advances (no register)",
  dayNet: "Net (selected day)",
  dayNetAccountingHint:
    "Cash out from register plus non-register advances for the day. Owner- and staff-pocket expenses are excluded here.",
  dayNetAfterAllRegisterOut: "Net (all register lines)",
  dayNetAfterAllRegisterOutHint:
    "Income minus every expense line dated this day (register, owner-paid, and staff-pocket).",
  registerSummarySectionCash: "Register & books (selected day)",
  registerSummarySectionCashHint:
    "Intake, cash out from the drawer, accounting net, and end-of-day result including every expense line.",
  registerSummarySectionPatron: "Owner balance (selected day)",
  registerSummarySectionPatronHint:
    "Owner-paid expenses create register debt; paying the owner from the drawer is a separate line. Net = first row minus middle row.",
  registerSummarySectionPocket: "Staff pocket (selected day)",
  registerSummarySectionPocketHint:
    "Register debt for pocket-paid expenses, minus repayments from the register or the owner.",
  registerSummarySectionBalance: "Cash accumulated in register",
  registerSummarySectionBalanceHint:
    "Physical cash through the selected date (cash in − out from register).",
  dayRegisterOwesPatron: "Owed to owner (owner paid)",
  dayRegisterOwesPatronHint:
    "Sum of expenses marked owner-paid; cash did not leave the drawer — the branch owes the owner.",
  dayRegisterOwesPatronShortHint:
    "Same-day total of expenses the owner paid on behalf of the branch (gross).",
  dayPatronDebtRepaidFromRegister: "Paid owner from register (day)",
  dayPatronDebtRepaidFromRegisterHint:
    "Cash that actually left the drawer to settle owner-paid expenses (dedicated expense line).",
  dayNetRegisterOwesPatron: "Net owed to owner (day)",
  dayNetRegisterOwesPatronHint:
    "Same day: owner-paid expenses minus cash paid to the owner from the register.",
  dayNetRegisterOwesPatronFormulaHint: "Owner debt above − paid from register row.",
  dayRegisterOwesPersonnel: "Owed to staff (pocket expenses)",
  dayNetRegisterOwesPersonnelPocket: "Net owed to staff (pocket)",
  dayNetRegisterOwesPersonnelPocketHint:
    "Same day: new pocket expenses minus pocket repayments from register or owner.",
  dayPocketRepaidRegisterShort: "Pocket repaid from register (day)",
  dayPocketRepaidPatronShort: "Pocket repaid by owner (day)",
  dayRegisterOwesPersonnelHint:
    "Today’s expenses marked as paid from staff pocket; physical register cash is unchanged.",
  dayNetCashHint: "Register net (day)",
  cashBalanceThrough: "Cash in register (through date)",
  cashBalanceThroughHint:
    "Physical cash: cash income minus expense rows through the selected date. BANK/PATRON advances count as expense only.",
  branchAdvancesTitle: "Advances at this branch",
  branchAdvancesHint:
    "Newest first (limit 500). CASH = leaves register; BANK/PATRON = branch expense without register.",
  advColDate: "Date",
  advColPerson: "Personnel",
  advColAmount: "Amount",
  advColSource: "Source",
  advColNote: "Note",
  noBranchAdvances: "No advances recorded for this branch.",
  txList: "Register lines for selected day",
  noTx: "No records for this date.",
  txColType: "Income / expense",
  txColAmount: "Amount",
  txColCashCard: "Cash / card",
  txColCashSettlement: "Physical cash (who holds)",
  txRegisterDayCloseHint:
    "Day-close: split into cash and card/POS (card reflects the bank). Mark physical cash — remitted to owner, with general or branch lead, or still in the drawer.",
  txDayClosePatronDebtRepayToggle:
    "Count this cash as repaying register debt to the owner (adds the automatic register payout line)",
  txDayClosePatronAutoDebtHint:
    "Turn off if cash went to the owner but should not reduce register debt to the owner. Deleting the day-close also removes the linked line when this is on.",
  txDayCloseOptionalExpenseTitle: "Same-day expense (optional)",
  txDayCloseBundledExpenseCheckbox: "Expense included",
  txDayCloseBundledExpenseCheckboxHint:
    "Opens with operating expense and paid from register as defaults. Fill the fields and use Confirm to add each line to the table (up to 3 per day close).",
  txDayCloseBundledExpenseConfirm: "Confirm expense",
  txDayCloseBundledExpenseListTitle: "Confirmed expenses",
  txDayCloseBundledExpenseTableType: "Type",
  txDayCloseBundledExpenseTableSub: "Subtype",
  txDayCloseBundledExpenseTablePay: "Payment",
  txDayCloseBundledExpenseTableAmount: "Amount",
  txDayCloseBundledExpenseTableNote: "Note",
  txDayCloseBundledExpenseRemove: "Remove",
  txDayCloseBundledExpenseMax: "You can add at most 3 same-day expenses with this day close.",
  txDayCloseBundledExpenseDraftNotConfirmed:
    "Clear the expense amount or confirm it with «Confirm expense» before saving.",
  txDayCloseBundledExpensePreviewLabel: "Expense line preview",
  txDayCloseBundledExpensePreviewIncomplete: "Select subtype and payment source to match what will be saved.",
  txDayCloseOptionalExpenseHint:
    "If you enter an amount, choose the expense type and whether it was paid from the branch register or by the owner; a matching expense line is saved for the same date.",
  txDayCloseOptionalExpenseAmount: "Expense amount",
  txDayCloseOptionalExpenseDescription: "Note (optional)",
  txDayCloseBundledExpenseMainLabel: "Expense type",
  txDayCloseBundledExpenseSubLabel: "Subtype",
  txDayCloseBundledExpenseMainRequired: "Select an expense type for the same-day expense.",
  txDayCloseBundledExpenseSubRequired: "Select a subtype for this expense.",
  txDayCloseBundledExpensePaymentRequired:
    "Select payment source: branch register or owner (required when an expense amount is entered).",
  txDayCloseBundledExpensePaymentHint:
    "Register: cash leaves the drawer. Owner: paid by the owner (not from register cash).",
  txDayCloseBundledExpenseFailedAfterIncome:
    "Income was saved, but the same-day expense could not be added:",
  txDayCloseBundledExpenseFailedBeforeIncome:
    "Day close was not opened because a same-day expense could not be added:",
  txDayCloseIncomeFailedAfterBundledExpense:
    "Same-day expense was saved, but the day close could not be completed:",
  cashSettlementHintSplit:
    "This line includes a cash amount; you can record whether physical cash was remitted to the owner, held by a general or branch lead, or left in the branch register. Card/POS amounts do not need this.",
  cashSettlementLabel: "Where is physical cash / who holds it?",
  cashSettlementUnset: "Not specified",
  cashSettlementPatron: "Remitted to owner",
  cashSettlementBranchManager: "With general or branch lead",
  cashSettlementResponsiblePerson: "Responsible personnel (general or branch)",
  cashSettlementResponsiblePick: "Select…",
  cashSettlementResponsibleEmpty:
    "No active personnel are assigned to this branch. Assign staff to the branch on the personnel card first.",
  cashSettlementResponsibleEmptyGlobal:
    "No active personnel found. Add personnel records or ensure they are not deleted.",
  cashSettlementRemainsAtBranch: "Still at branch (in register)",
  registerCashSettlementTitleSingle: "Edit cash handover",
  registerCashSettlementTitleBulk: "Bulk cash handover (day close / register intake)",
  registerCashSettlementLeadSingle:
    "Update where physical cash is recorded for this income line. Transaction # and date:",
  registerCashSettlementLeadBulk:
    "Apply the same cash handover to all day-close and register-intake income lines in the selected date range. Range:",
  registerCashSettlementHintPatronBulk:
    "Bulk cannot set «Remitted to owner» (owner debt repayment lines are per day). Edit those rows one by one.",
  registerCashSettlementNeedPerson: "Select the responsible person.",
  registerCashSettlementBulkDatesInvalid: "Enter both start and end dates (YYYY-MM-DD) for bulk update.",
  registerCashSettlementSaved: "Cash handover updated.",
  registerCashSettlementBulkDone: "Bulk cash handover completed.",
  registerCashSettlementBulkPartial: "Bulk update finished with some rows skipped or failed.",
  registerCashSettlementOpenSingle: "Cash handover",
  registerCashSettlementOpenBulk: "Bulk handover in range",
  registerCashSettlementColShort: "Cash",
  registerCashSettlementSelectAll: "Select all",
  registerCashSettlementRowsTitle: "Rows to update",
  registerCashSettlementCandidatesLoading: "Loading rows…",
  registerCashSettlementCandidatesEmpty: "No eligible income lines in this date range.",
  registerCashSettlementNeedSelection: "Select at least one row.",
  registerCashSettlementSelectedCount: "{n} row(s) selected",
  registerCashSettlementExistingBadge: "Previously handed over",
  registerCashSettlementExistingRowsHint:
    "{n} rows already have a cash handover; selecting them will overwrite with the new choice.",
  txColMainCategory: "Main category",
  txColCategory: "Category",
  txColExpensePayment: "Payment source",
  txColNote: "Note",
  txColCreatedBy: "Created by",
  txColKind: "Type",
  txDetailSectionAudit: "Record info",
  expenseKindAdvance: "Advance",
  expenseKindPersonnel: "Personnel expense",
  expenseKindOverhead: "General overhead",
  expenseKindInvoice: "Invoice",
  expenseKindBranch: "Branch expense",
  txTypeIn: "Income",
  txTypeOut: "Expense",
  staffTitle: "Personnel at this branch",
  staffNone: "No personnel assigned to this branch.",
  staffName: "Name",
  staffAdvTotal: "Total advances",
  personnelMoneyColAdvances: "Advances (branch, all-time)",
  personnelMoneyColRegisterOwes: "Register owes (pocket)",
  personnelMoneyColPocket: "Pocket summary",
  personnelMoneyMixedCurrency: "Multiple currencies",
  personnelMoneyPocketOutShort: "Paid from pocket",
  personnelMoneyPocketRepaidShort: "Repaid to pocket",
  personnelMoneyPocketClaimTransferShort: "On-books reassignment (no cash out)",
  personnelMoneyPocketAheadShort: "Net ahead (over-repaid)",
  personnelPayPocketDebt: "Pay register debt (pocket)",
  personnelPayPocketDebtHint:
    "Opens expenses: staff pocket repayment — select settled lines, pay from register or owner.",
  staffAdvCount: "Advances (#)",
  staffAdvHistory: "Advances (this branch)",
  staffGiveAdvanceRow: "Advance",
  openPersonnel: "Personnel",
  assignPersonnelOpen: "Assign to this branch",
  assignPersonnelTitle: "Assign personnel to this branch",
  assignPersonnelHint:
    "People not already on this branch are listed with their current branch. Use the checkboxes to select one or more. Unassigned staff are added without an extra confirmation.",
  assignPersonnelFilter: "Filter by name",
  assignPersonnelCurrentBranch: "Current branch",
  assignPersonnelNoBranch: "Unassigned",
  assignPersonnelEmpty: "No one available to assign (everyone is already on this branch).",
  assignPersonnelPick: "Select",
  assignPersonnelAdd: "Add to branch",
  assignPersonnelConfirmBody:
    "«{name}» is currently under «{fromBranch}». They will be moved to «{toBranch}». Do you want to continue?",
  assignPersonnelConfirmBack: "Back",
  assignPersonnelConfirmSubmit: "Yes, move",
  assignPersonnelSelectAll: "Select all",
  assignPersonnelClearSelection: "Clear selection",
  assignPersonnelSelectedCount: "{n} selected",
  assignPersonnelConfirmBatchTransfersTitle: "Moving from another branch:",
  assignPersonnelConfirmBatchFooter:
    "Everyone you selected will be saved under «{toBranch}». Do you want to continue?",
  assignPersonnelConfirmBatchAlsoUnassigned:
    "Also, {n} unassigned staff will be added to this branch.",
  assignPersonnelStepPickLabel: "Pick staff",
  assignPersonnelStepConfirmLabel: "Confirm",
  assignPersonnelConfirmListTitle: "Staff to be moved",
  assignPersonnelConfirmSummary:
    "{n} staff will be saved under «{toBranch}».",
  assignPersonnelConfirmRowFromTo: "{from} → {to}",
  giveAdvance: "Give advance",
  totalAdvancesBranch: "Total advances (this branch)",
  totalAdvancesBranchPeriod:
    "Advances for effective period (month of selected day, this branch)",
  txModalTitle: "New income / expense",
  txModalTitleNewIncome: "New income",
  txModalTitleNewExpense: "New expense",
  txModalTitleDayClose: "Close day (register)",
  txModalTitlePocketRepay: "Staff pocket: settle debt",
  txModalTitlePocketClaimTransfer: "Pocket claim transfer",
  txModalTitleHandoverPatron: "Settle owner debt (from cash handover)",
  txModalTitleHandoverExpense: "Expense from cash handover",
  txModalHintNewIncome: "Add an income line to this branch.",
  txModalHintNewExpense: "Add an expense line to this branch.",
  txModalHintDayClose: "Record this day's register close.",
  txModalHintPocketRepay: "Settle a staff member's pocket debt to the register.",
  txModalHintPocketClaimTransfer: "Transfer a pocket claim to another staff member or to the owner.",
  txModalHintHandoverPatron: "Settle an owner debt from cash handover.",
  txModalHintHandoverExpense: "Record an expense paid from a cash handover.",
  txOrgModalTitle: "Organization expense (no branch)",
  txOrgModalHintShort:
    "Expense not tied to a branch register. Payment source must be owner (PATRON) or staff pocket (register debt).",
  txOrgPersonnelModalTitle: "Personnel expense",
  txOrgPersonnelModalHintShort:
    "An expense attributed to a specific employee, paid directly from the patron's pocket without a branch assignment.",
  txOrgAdvanceExistingOnly: "Organization expenses cannot create new advances here; link an existing advance only.",
  txOrgPerOtherNeedPersonnel: "Select personnel for this expense (personnel card).",
  txModalHintShort: "Add an income or expense line to this branch register.",
  txModalHintDetail:
    "Not stock moves — money in or out of the register. For income, use cash and card fields to split POS/bank vs cash in hand.",
  txExpenseRoutingTitle: "Where to record it (recommended)",
  txExpenseRoutingIntro:
    "Use the right screen first — reports and supplier links stay consistent.",
  txExpenseRoutingSupplierInvoices: "Formal supplier invoice (goods or services)",
  txExpenseRoutingSupplierHrefLabel: "Open supplier invoices",
  txExpenseRoutingPersonnelCosts:
    "Per-personnel costs (salary, bonus, advance, commute…): here or Personnel › Costs; you can link an advance to this line later",
  txExpenseRoutingPersonnelHrefLabel: "Open personnel costs",
  txExpenseRoutingVehicleCosts:
    "Fleet / plate expenses (fuel, etc. hitting the branch register): record under Vehicles; the register line is created automatically",
  txExpenseRoutingVehicleHrefLabel: "Open Vehicles",
  txExpenseRoutingBranchForm:
    "This form is for day-to-day register operating costs (rent, utilities, taxes, POS, meals, franchise, small non-invoiced buys…). Use the links above for fleet costs and formal supplier invoices.",
  txRoutingPersonnelOnlyTitle: "Personnel expense from this screen",
  txRoutingPersonnelOnlyBody:
    "You are posting a personnel line on the register. You can pick advance / payroll links here, or enter from Personnel › Costs first and link afterward.",
  txRoutingHintOutGoods:
    "Purchases tied to a supplier invoice should be entered under Supplier invoices (stock and branch allocation). Use Goods & stock here only for simple cash purchases without an invoice, if your process allows.",
  txRoutingHintOutOps:
    "Formal supplier bills live under Supplier invoices. For plated vehicle costs, use Vehicles › post to branch (fuel lines originate there); use Operations below for on-site costs (e.g. generator fuel) when needed.",
  txRoutingHintOutPersonnel:
    "Per-personnel expense: here or Personnel › Costs. You can attach advance / payroll links when saving or add them later.",
  txNotifyIncomplete: "Please fill in the fields marked with *.",
  txAmountTotalLabel: "Total amount",
  txDayCloseBundledExpenseAddAnother: "Add another expense",
  expenseHeldRegisterPickerCrossBranchHint:
    "Showing held register cash balances aggregated across all branches.",
  txPersonnelExpenseHeroLabel: "Personnel Expense",
  txPersonnelExpenseHeroInfoAria: "Personnel expense info",
  txPersonnelExpenseHeroInfoBody:
    "Choosing branch register deducts from register cash; owner or held cash does not. Pick a sub-type and payment source; effective year/season is attributed automatically.",
  txDayCloseHeroLabel: "Day-End Close",
  txDayCloseHeroLine: "closing this day financially",
  txDayCloseHeroInfoAria: "Day-end info",
  txDayCloseHeroInfoBody:
    "Once confirmed, this day will no longer accept new register expenses or income. Any remaining same-day expenses can still be added from the «same-day expense» panel below.",
  txStoryIncomeCallout:
    "Income is money received at the branch. Card/POS typically settles at the bank; for cash in hand you can optionally mark where that physical cash ended up.",
  txStoryExpenseCallout:
    "Personnel expense: a purchase made for the staff member (clothing, groceries, shopping, etc.) — not cash handed or transferred to them directly. Direct payments belong in the Advances flow.",
  txType: "Entry type",
  txAmount: "Amount",
  txCurrency: "Currency",
  txAmountInvalid: "Enter a valid amount greater than zero.",
  txDateField: "Date",
  txMainCategory: "Main category",
  txSubCategory: "Category",
  txSelectPlaceholder: "Select…",
  txMainInSales: "Sales revenue",
  txMainInService: "Service revenue",
  txMainInOther: "Other income",
  txMainInPatron: "Cash from owner (register owes)",
  txMainInDayClose: "Day-close (register summary)",
  txSubPatronCash: "Cash received from owner",
  txPatronCashIncomeHint:
    "Cash the owner puts into the branch register: counts as income and increases what the register owes the owner (same side as owner-paid expenses). Enter one amount only; no cash/card split.",
  txPatronCashNoSplit: "Owner cash income cannot use cash/card split; enter a single total amount.",
  txMainOutPersonnel: "Personnel",
  txMainOutGoods: "Goods & stock",
  txMainOutOps: "Operations",
  txMainOutCapex: "Capital / fixtures",
  txMainOutTax: "Taxes & statutory deductions",
  txMainOutPocketRepay: "Pocket debt repayment to staff",
  txMainOutPocketClaimTransfer: "Reassign register-owed amount (book entry)",
  txMainOutPatronDebtRepay: "Repay owner from register",
  txSubContractorPayment: "Contractor payment",
  txSubPocketRepay: "Pocket repayment",
  txSubPocketClaimTransfer: "Staff → staff (same booking obligation)",
  txSubPocketClaimTransferToPatron: "Staff → owner (reassigned on the books)",
  txPocketClaimTransferModalHint:
    "Changes who the register books as owing the «paid from pocket» amount (no cash leaves the drawer). Use staff→staff to move the obligation between people, or staff→owner when the staff member reassigns it to the owner on paper. To pay out cash for pocket-paid expenses, use «Pocket debt repayment to staff».",
  txPocketClaimTransferNeedBranch: "This booking reassignment must be recorded on a branch.",
  txPocketClaimTransferFromRequired: "Select who is giving up the booking (from).",
  txPocketClaimTransferSamePerson: "From and to personnel must be different.",
  txPocketClaimTransferFromLabel: "From (gives up register-owed booking)",
  txPocketClaimTransferToLabel: "To (takes over that register-owed booking)",
  txSubPatronDebtRepay: "Owner debt repayment",
  txPocketRepayModalHint:
    "Pay from register or owner to the staff member, then tick the pocket-paid expense lines you are settling (required; amount comes only from the selection).",
  txPatronDebtRepayModalHint:
    "Record cash actually leaving the register to settle expenses the owner paid earlier; payment source is register only.",
  txMainOutOther: "Other expense",
  txMainOutNonPnl: "Non–P&L memo (franchise / partner POS)",
  txSubNonPnlMemo: "Memo line",
  txNonPnlModalHint:
    "Does not affect branch income/expense totals or register math. Use for POS/partner flows (e.g. franchise) you only need to see on the branch card.",
  txNonPnlBadge: "Outside P&L",
  txSubSaleRetail: "Retail sale",
  txSubSaleWholesale: "Wholesale sale",
  txSubSvcConsult: "Consulting / fee",
  txSubSvcRepair: "Repair / service work",
  txSubIncOther: "Other income",
  txAmountCash: "Cash",
  txAmountCard: "Card (POS / bank)",
  txCashShort: "Cash",
  txCardShort: "Card",
  txAmountSplitHint:
    "Recommended: enter cash (in drawer) and card/POS separately; total must equal cash + card. That unlocks cash handover tracking for the cash portion.",
  txSplitIncomplete: "Enter both cash and card amounts, or leave both empty and use total only.",
  txSubPerSalary: "Salary-related",
  txSubPerBonus: "Bonus / premium",
  txSubPerAdvance: "Advance repayment",
  txSubPerOvertime: "Overtime pay",
  txSubPerCommission: "Commission / incentive pay",
  txSubPerMealAllowance: "Meal allowance",
  txSubPerTransport: "Transport / commute",
  txSubPerUniform: "Uniform / work clothes",
  txSubPerHealth: "Health (check-ups, insurance top-ups)",
  txSubPerSocialSecurity: "Social security / employer statutory contributions",
  txSubPerTraining: "Training / certification",
  txSubPerSeverance: "Severance / notice pay",
  txSubPerOther: "Other personnel",
  txExpenseLinkAdvance: "Link to advance",
  txExpenseLinkSalary: "Link to salary / bonus payment",
  txExpenseLinkPersonnelLabel: "Personnel (whose payment)",
  txExpenseLinkPersonnelPick: "Select personnel…",
  txExpenseLinkPersonnelRequired: "Select personnel before linking an advance or salary payment.",
  txExpenseLinkPickPersonnelFirst: "Select personnel to list their open advance / salary records.",
  txAdvanceModeLabel: "Advance",
  txAdvanceModeExisting: "Link existing advance (from register)",
  txAdvanceModeNew: "New advance from register",
  txAdvanceModePersonnelPocket: "Paid from personnel pocket (register owes them)",
  txAdvanceNewHint:
    "Creates the advance (season year) and one linked register expense line so it appears in reports for the transaction date you set below. Reports filter by that date, not by season year alone.",
  txAdvanceCreatedRegisterFailed:
    "Advance was saved, but the register expense line failed:",
  txAdvancePocketHint:
    "Only the advance record is created: no cash leaves the register yet. When you pay from the register, add a personnel expense and link this advance.",
  txAdvanceEffectiveYear: "Season year (defaults to current year)",
  txAmountLockedHint: "Amount and currency follow the selected record.",
  txExpenseLinkPick: "Select…",
  txExpenseLinkRequired: "Select the linked payment record.",
  txExpenseLinkEmpty: "No unlinked records for this branch.",
  txLinkedAdvance: "Linked advance",
  txLinkedSalary: "Linked salary payment",
  txLinkedSupplierInvoiceLine: "Supplier invoice line",
  txLinkedVehicle: "Vehicle expense",
  txLinkedContractor: "Contractor",
  txGeneralOverheadPool: "General overhead pool",
  txSubStkPurchase: "Stock purchase",
  txSubStkMaterial: "Materials / consumables",
  txSubStkOther: "Other goods",
  txSubCapexEquipment: "Equipment (AC, camera, POS)",
  txSubCapexFurniture: "Furniture / fixtures",
  txSubCapexFitout: "Fit-out / decoration",
  txSubCapexOther: "Other capital",
  txSubOpsRent: "Rent",
  txSubOpsUtil: "Utilities",
  txSubOpsMaintenance: "Maintenance / repair",
  txSubOpsMarketing: "Marketing / advertising",
  txSubOpsCleaning: "Cleaning",
  txSubOpsSupplies: "Office supplies",
  txSubOpsInsurance: "Insurance",
  txSubOpsConsulting: "Consulting (accounting / legal)",
  txSubOpsSecurity: "Security",
  txSubOpsSoftware: "Software / subscription",
  txSubOpsTraining: "Training / seminar",
  txSubOpsTravel: "Travel / lodging",
  txSubOpsHospitality: "Hospitality / gifts",
  txSubOpsBankFee: "Bank fees (transfers / account)",
  txSubOpsLicense: "License / permit",
  txSubOpsTax: "Tax",
  txSubOpsFranchise: "Franchise (royalty / marketing)",
  txSubOpsMeal: "Meals",
  txSubOpsFuel: "Fuel",
  txSubOpsCargo: "Shipping / cargo",
  txSubOpsOther: "Other operations",
  txSubTaxWithholding: "Withholding tax (stopaj)",
  txSubTaxPosEod: "POS day-end tax / settlement levy",
  txSubTaxVat: "VAT (payment / settlement)",
  txSubTaxStamp: "Stamp duty",
  txSubTaxMunicipal: "Municipal / environmental levies",
  txSubTaxExcise: "Excise / special consumption (ÖTV)",
  txSubTaxSsi: "SSI / unemployment (employer statutory)",
  txSubTaxOther: "Other tax / levy",
  invoicePaymentStatusLabel: "Invoice payment",
  invoicePaymentUnpaid: "Not paid yet (no cash out)",
  invoicePaymentPaid: "Paid (cash / patron / pocket)",
  invoiceUnpaidBadge: "Invoice · unpaid",
  invoiceSettleTitle: "Record invoice payment",
  invoiceSettleHint: "Cash leaves the register, owner, or staff pocket according to your choice.",
  invoiceSettleSubmit: "Save payment",
  txSubExpOther: "Other expense",
  txLegacySale: "Sale (system)",
  txLegacyExpense: "Expense (system)",
  txLegacyAdvance: "Advance (personnel)",
  txLegacySalary: "Salary payment",
  txCategoryUnknown: "Unspecified category",
  txDescription: "Note (optional)",
} as const;
