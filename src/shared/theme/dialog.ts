...
export const dialogTheme = {
  ...
  /** iOS klavye uyumlu: panel scroll'lu değil; içerik dahili scrollable wrapper'da. Modal’a `bodyScroll` ile aktiflenir. */
  panelBodyScroll:
    "flex max-h-[min(92dvh,calc(100svh-max(env(safe-area-inset-top),0.5rem)-max(env(safe-area-inset-bottom),0.5rem)-0.5rem))] w-full max-w-[min(30rem,calc(100vw-1rem))] flex-col overflow-hidden rounded-2xl border border-zinc-200/90 bg-white shadow-2xl shadow-zinc-900/12 ring-1 ring-zinc-900/[0.04] sm:max-w-md md:max-w-lg lg:max-w-3xl xl:max-w-5xl 2xl:max-w-6xl",
