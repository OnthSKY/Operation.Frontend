...
export function PersonnelCostsScreen() {
  ...
  return (
    <div className="mx-auto flex w-full app-page-max flex-col gap-4 p-4 pb-8">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl font-semibold leading-tight tracking-tight text-zinc-900 sm:text-xl">
            {t("personnel.costsTitle")}
          </h1>
          <p className="mt-1 text-sm text-zinc-500">{t("personnel.costsDesc")}</p>
        </div>
        {!personnelPortal ? (
          <div className="grid w-full grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 lg:max-w-none lg:w-full xl:w-auto xl:min-w-0">
            <Button
              type="button"
              variant="secondary"
            ...
            >
              <CostsIconExpense />
              <span className="min-w-0 text-center">
                {t("personnel.costsAddPersonnelExpense")}
              </span>
            </Button>
            <Button
              type="button"
              variant="secondary"
              className="min-h-12 w-full justify-center gap-2 sm:col-span-2 lg:col-span-1"
      ...
      {!personnelPortal ? (
        <div
          className="flex flex-wrap gap-2 rounded-xl border border-zinc-200 bg-zinc-50/80 p-2"
          role="tablist"
      ...
      {!personnelPortal && (
        <Card title={t("personnel.costsSummaryTitle")}>
          {advancesPending || expensesQuery.isPending ? (
            <p className="text-sm text-zinc-500">{t("common.loading")}</p>
          ) : (
            <div className="space-y-4 text-sm">
              <p className="text-xs text-zinc-500">{t("personnel.costsSummaryHint")}</p>
              {sortedCurrencyKeys(advSums, expSums).map((ccy) => {
                ...
                return (
                  <div
                    key={ccy}
                    className="grid gap-2 rounded-lg border border-zinc-100 bg-zinc-50/50 p-3 sm:grid-cols-3"
                  >
                    <div>
      ...
      >
        ...
        {!personnelPortal ? (
          <p className="mt-3 text-xs text-zinc-500">
            {t("personnel.costsFiltersFootnote")}
          </p>
        ) : null}
        <p className="mt-2 text-xs text-zinc-500">
          {t("personnel.costsFilterPaymentFromHint")}
        </p>
      </CollapsibleMobileFilters>

      <Card title={t("personnel.costsUnifiedTableTitle")}>
